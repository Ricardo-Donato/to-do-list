    ...
    if (contactForm) {
        ...
        contactForm.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                if (result.success) {
                    contactForm.reset();
                    messageContainer.textContent = 'Mensagem enviada! Em breve daremos retorno.';
                    messageContainer.className = 'success';
                    setTimeout(() => {
                        window.location.replace("index.html");
                    }, 4000);
                } else {
                    messageContainer.textContent = result.error || 'Erro ao enviar mensagem.';
                    messageContainer.className = 'error';
                }
            } catch (err) {
                messageContainer.textContent = 'Erro ao enviar mensagem.';
                messageContainer.className = 'error';
            }
